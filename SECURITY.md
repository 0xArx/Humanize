...
|-------|----------------|------------|
| Identity file (service keys, addresses) | `~/.humanize/identity.json` | Mode 600 in a mode 700 folder. Written atomically under a lock, with a `.bak` of the previous version. Never committed (`.gitignore`). |
...
| The self key | `~/.humanize/self.key` (mode 600), and with the human | Shown once. Cannot be recovered. |
...
- **The self key is stored next to the identity** so pushes work unattended. It protects the copy in the git repo, not the copy on this disk.
